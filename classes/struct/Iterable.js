...
export class ArrayBuilder extends Builder {
	...
	addOne(item) { this._buffer.push(item); }
	result() {
...
export class BuilderWrapper extends Builder {
	...
	addOne(item) { this._wrapped.addOne(item); }
	result() { return this._wrap(this._wrapped.result()); }
}
...
export class Iterable {
	...
	// f should accept value and return true or false
	forall(f) { return !this.exists(a => !f(a)); }
	...
	mkString(separator = '') { return this.iterator().mkString(separator); }
	...
	to(builder = this.newBuilder()) { return this.iterator().to(builder)  }
